...
function confirmacao() {
  return {
    components: [new ContainerBuilder()
      ...
      .addTextDisplayComponents(new TextDisplayBuilder().setContent('Este usuário agora não é confiável no seu canal de voz.'))],
  };
}

export async function desconfiarSelect(interaction) {
  ...
  const selectedIds = new Set(interaction.values.filter((id) => doc.trustedUsers.includes(id)));
  ...
  await doc.save();
  logger.info('PAINEL', interaction.user.username, `Removeu ${selectedIds.size} usuário(s) dos confiáveis.`);
